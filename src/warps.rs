...
fn test_diamond_partitioning() {
    let a: Vec<f64> = (0..310).map(|_| rand::random::<f64>()).collect();
    let b: Vec<f64> = (0..771).map(|_| rand::random::<f64>()).collect();
    ...
    let res = diamond_partitioning(
        ...
        f64::INFINITY,
        |i, j, x, y, z| {
            (a.get(i).copied().unwrap_or(0.0) - b.get(j).copied().unwrap_or(0.0)).powi(2)
                + x.min(y.min(z))
        },
    );
...
pub fn diamond_partitioning(
    ...
    for i in 0..rows_count {
        ...
        for j in 0..diamonds_count {
            ...
            println!(
                "Diamond: {} {} {}",
                (diag_start as isize - offset as isize),
                d_a_start,
                d_b_start
            );
        }
