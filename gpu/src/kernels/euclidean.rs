...
mod kernels {
    ...
    pub fn euclidean_impl(x1: f32, x2: f32, y: &mut f32) {
        *y += (x1 - x2) * (x1 - x2);
    }

    #[kernel]
    pub fn euclidean(#[item] x1: f32, #[item] x2: f32, #[item] y: &mut f32) {
        euclidean_impl(x1, x2, y);
    }

    #[kernel]
    pub fn euclidean_global(
        #[global] x1: Slice<f32>,
        #[global] x2: Slice<f32>,
        ...
        if global_id < x1.len().min(x2.len()).min(y.len()) {
            euclidean_impl(x1[global_id], x2[global_id], unsafe {
...
fn euclidean(x1: Slice<f32>, x2: Slice<f32>, mut y: SliceMut<f32>) -> Result<()> {
    if false {
        // use local threads
        println!("local");
        kernels::euclidean::builder()?
            .build(y.device())?
            .dispatch(x1, x2, y);
    } else {
        // or
        println!("global");
        kernels::euclidean_global::builder()?
            .build(y.device())?
...
pub fn euclidean_distance(x1: Vec<f32>, x2: Vec<f32>) -> f32 {
